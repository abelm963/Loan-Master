...
async function onRecordPayment_(event) {
  event.preventDefault();
  await saveLoanTransaction_(event.currentTarget, 'Payment');
}

async function onRecordWaiver_(event) {
  event.preventDefault();
  await saveLoanTransaction_(event.currentTarget, 'Interest Waiver');
}

async function onRecordAdjustment_(event) {
  event.preventDefault();
  await saveLoanTransaction_(event.currentTarget, 'Adjustment');
}
    ...
    .map((loan) => {
      ...
      return `
        <article class="list-item clickable" data-loan-id="${escapeHtml_(loan.LoanID)}">
          ...
          <span class="muted">${formatCents_(loan.OutstandingCents)} • Due ${escapeHtml_(loan.DueDate || '-')}</span><br />
...
function renderLoanDetail_(payload) {
  ...
  const header = `
    <div class="list-item">
      ...
      <span class="muted">Outstanding: ${formatCents_(payload.outstandingCents)} • Due ${escapeHtml_(loan.DueDate || '-')}</span><br />
  ...
  const rows = txs
    .map(
      ...
      <div class="list-item">
        ...
        <span class="muted">${escapeHtml_(tx.Date || '')} • ${escapeHtml_(tx.Notes || '')}</span>
      </div>
    `
    )
    .join('');
